...
def test_filter_with_today_data():
    ...
    try:
        ...
        for idx, row in df.iterrows():
            print(
                f"{idx + 1}. [{row['机号']}] {row['航班号']} - {row['触发时间']} - {row['描述']} - 阶段:{row['飞行阶段']}"
            )
    ...
    if not filter_obj.group_rules.empty:
        ...
        for idx, rule in filter_obj.group_rules.iterrows():
            ...
            if fault_descs:
                print(
                    f"   规则 {idx + 1}: {rule.get('规则名称', '未命名')} - 同时出现: {' + '.join(fault_descs)}"
                )
    ...
    if len(filtered_df) < len(df):
        ...
        for idx, row in filtered_df.iterrows():
            ...
            print(
                f"{idx + 1}. [{row['机号']}] {row['航班号']} - {trigger_time} - {row['描述']} - 阶段:{row['飞行阶段']}"
            )
    else:
        print("\nℹ️  没有故障被过滤")

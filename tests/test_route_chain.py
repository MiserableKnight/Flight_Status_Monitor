...
def test_route_chain_config():
    ...
    if is_last:
        print(f"  ✅ 正确! VJ106回到河内,完成航线A")
    else:
        print(f"  ❌ 错误! VJ106应该是航线A的最后航班")

    is_last = FlightSchedule.is_last_flight_in_route('VJ118')
    print(f"\n✓ VJ118 是否为航线最后航班: {is_last}")
    if not is_last:
        print(f"  ✅ 正确! VJ118不是最后,后面还有VJ119和VJ108")
    else:
        print(f"  ❌ 错误! VJ118不是航线B的最后航班")

    is_last = FlightSchedule.is_last_flight_in_route('VJ108')
    print(f"\n✓ VJ108 是否为航线最后航班: {is_last}")
    if is_last:
        print(f"  ✅ 正确! VJ108回到河内,完成航线B")
    else:
        print(f"  ❌ 错误! VJ108应该是航线B的最后航班")
...
def demonstrate_fix():
    ...
    chain = FlightSchedule.get_route_chain('VJ118')
    print(f"   - 识别航线链: {chain}")
    print(f"   - 判断: VJ118已完成,但不是航线链最后")
    print(f"   - 后续: VJ119 (胡志明→昆岛), VJ108 (昆岛→河内)")
    print(f"   - 结果: 'B-656E停靠胡志明；计划执行VJ119。' ✅")
    ...
    chain = FlightSchedule.get_route_chain('VJ106')
    print(f"   - 识别航线链: {chain}")
    print(f"   - 判断: VJ106已完成,且是航线链最后")
    print(f"   - 结果: 'B-652G停靠河内；已完成今日所有航班。' ✅")
